...
export default function DiscoverPage() {
  ...
  const fetchKitchen = useCallback(async () => {
    setLoading(true);
    try {
      const response = await api.get("user/kitchens/auto-select");

      if (response.data.success && response.data.kitchen) {
        const k = response.data.kitchen;

        const mapped = [
          {
            id: k._id,
            name: k.restaurantInfo?.name || "Unnamed Kitchen",
    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="bg-gradient-to-r from-slate-900 to-slate-800 text-white py-6">
        <div className="max-w-7xl mx-auto px-6 flex-col">
          <h1 className="text-4xl font-black">
            Auto Selected <span className="text-orange-400">Kitchen</span>
          </h1>
          <p className="text-slate-300 mt-2 max-w-xl">
            We found the best kitchen near you automatically.
          </p>
        </div>
      ...
      <main className="max-w-7xl mx-auto px-6 py-8">
        {loading ? (
          ...
          <div className="bg-orange-50 border border-orange-100 p-6 rounded-xl text-orange-700 font-medium">
            No kitchen available near you right now.
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
            {restaurants.map((r) => (
              ...
              >
                ...
                <div className="p-4 flex flex-col flex-grow">
                  ...
                  <div className="mt-3 text-xs text-slate-500">
                    <p>💰 Base Fee: ₹{r.pricing?.baseFee}</p>
                    <p>📏 Per Km Charge: ₹{r.pricing?.perKmCharge}</p>
                  </div>
                  ...
                  <div className="mt-auto pt-4">
                    <button
                      ...
                      className="w-full bg-slate-900 text-white py-2 rounded-lg font-bold hover:bg-orange-600 transition"
                    >
                      View Menu
                    </button>
                  </div>
